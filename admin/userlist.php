...
{
	$sql = 'SELECT * FROM '.$pdodb['prefix'].'users WHERE CONCAT( firstname,lastname, username ) LIKE \'%'.PDO::qote($_GET['search']).'%\' and status=2';
...
{
	// list all users
	$sql = 'SELECT * FROM '.$pdodb['prefix'].'users WHERE status=2 ORDER BY lastname';
...
if ($result = $dbh->query($sql2))
{
	if ($result->rowCount() > 0)
	{
		...
		while ($row = $result->fetch(PDO::FETCH_ASSOC)) {
			$userlist .= '<tr class="style2"><td>'.$row['lastname'].', '.$row['firstname'].'</td><td>'.$row['username'].'</td><td>'.str_chop($row['email'],20).'</td><td><a href="./userinfo.php?db_config_path=' . $db_config_path . '&user='.$row['username'].'"><img src="../images/info15px.gif" alt="Info" border="0" title="More Information" /></a> <a href="./bulkemail.php?db_config_path=' . $db_config_path . '&user='.$row['username'].'"><img src="../images/mail15px.gif" alt="Email" border="0" title="Email" /></a> <a href="#" onclick="deleteuser(\''.$row['username'].'\')"><img src="../images/delete15px.gif" alt="Remove" border="0" title="Remove" /></a></tr>'."\n";
		}
	}
	else
	{
...
<title>RegPortal - Manager Users</title>
...
<table width="549" border="0" align="center" cellpadding="0" cellspacing="0">
  ...
  <tr>
    <td height="20" colspan="2" class="style2"><strong><a href="./index.php?db_config_path=<?php echo $db_config_path; ?>">Top</a>: Manage Users</strong></td>
  </tr>
  <tr>
    <td height="28" colspan="2" class="style2">This is a list of all users in the database. You can click a user's name to view more information about that user, to remove their account, or to send them an email. You may also enter a first name, last name OR username into the search box to find a user.</td>
  </tr>
  <tr>
    <td height="19" colspan="2"><br />
        ...
        <br />
        <a href="./newuser.php?db_config_path=<?php echo $db_config_path; ?>" class="style2">Create New User&gt;&gt;</a><br /><br />
